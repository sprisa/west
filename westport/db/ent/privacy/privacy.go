...
// The LighthouseQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type LighthouseQueryRuleFunc func(context.Context, *ent.LighthouseQuery) error

// EvalQuery return f(ctx, q).
func (f LighthouseQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.LighthouseQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.LighthouseQuery", q)
}

// The LighthouseMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type LighthouseMutationRuleFunc func(context.Context, *ent.LighthouseMutation) error

// EvalMutation calls f(ctx, m).
func (f LighthouseMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.LighthouseMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.LighthouseMutation", m)
}
